...
import { getAuth, createUserWithEmailAndPassword, signInWithEmailAndPassword, GoogleAuthProvider, signInWithRedirect, getRedirectResult, onAuthStateChanged, signOut } from "https://www.gstatic.com/firebasejs/11.0.2/firebase-auth.js";
...
// Handle redirect results after returning to the page
const handleRedirectResult = async () => {
    try {
        const result = await getRedirectResult(auth);
        if (result) {
            const user = result.user;
            console.log('Redirect successful, user signed in:', user);
            ...
            if (!emailExists) {
                ...
                // Optionally redirect to the home page or update the UI
            }
        } else {
            console.log('No redirect result, user may not be signed in.');
        }
    } catch (error) {
